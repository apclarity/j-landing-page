...
let images = [];
for(let i=1; i<=28; i++){
    let img = '../../images/companyprofile/00'
    if(i < 10){
        img += '0'
    }
    img += i.toString() + ".jpg"
...
<template>
    <ModalBasic :modalOpen="props.modalOpenStatus" @close-modal="closeModal()"
    class="opacity-0"
    >
        <div>
            <flipbook class="flipbook" :pages="images">
                <div class="close">
                    <button class="block sm:hidden h-10 px-2 button-link mt-10 text-center items-center" @click="emit('closeModal')" >
